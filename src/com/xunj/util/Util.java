...
public class Util {
	...
	/**
	 * 复制相同名称属性，支持数据类型自动转换
	 * String to Integer 使用方法 StrToInteger(String s)
	 * Integer to String 使用方法 ObjectToString(Object src)
	 * String to Long 使用方法 StrToLong(String s)
	 * Long to String 使用方法 ObjectToString(Object src)
	 * String to Double 使用方法 StrToDouble(String s)
	 * Double to String 使用方法 ObjectToString(Object src)
	 * Sting to Date 使用方法 StringtoDate(String date) 
	 * 默认转换形式yyyy-MM-dd，可以通过setDateformat(String dateformat)形式更改转换格式
	 * Util.datefmt;默认格式
	 * Util.datetimefmt;日期时间格式
	 * Date to String 使用方法 DatetoString(Date dt) 转换格式同上
	 * @param src 源对象
	 * @param dest 目标对象
	 ...
	 * 5.IllegalAccessException异常  当应用程序试图创建一个实例（而不是数组）、设置或获取一个字段，或者调用一个方法，但当前正在执行的方法无法访问指定类、字段、方法或构造方法的定义时，抛出 IllegalAccessException。
	 * 6.InvocationTargetException异常 是一种包装由调用方法或构造方法所抛出异常的经过检查的异常。
	 * 7.NoSuchMethodException异常  无法找到某一特定方法时，抛出该异常
	 * 8.PropertyUtils.getPropertyDescriptors(对象) 将对象的属性返回成PropertyDescriptor[]数组
	 * 9.PropertyUtils.setProperty(对象，对象属性名称，对象属性值) //将参数中的对象属性值赋进参数中的对象属性中
				...
				for (int j = 0; j < pddest.length; j++) {
					...
					if (pdsrc[i].getName().equals(pddest[j].getName())) {
						...
						if (pdsrc[i].getPropertyType() == pddest[j]
							...
							// 将源对像属性值复制到目标对象属性值里面
							try {
								PropertyUtils.setProperty(dest, pddest[j]
										.getName(), PropertyUtils.getProperty(
										src, pdsrc[i].getName()));
	...
	public void copyMapToBean(HashMap src, Object dest) {
		...
		for (int j = 0; j < pddest.length; j++) {
			String value=(String) src.get(pddest[j].getName());
				if (pddest[j].getPropertyType().getName().equals("java.lang.String") ) {
					try {
						PropertyUtils.setProperty(dest, pddest[j]
								.getName(), value);
					} catch (IllegalAccessException e) {
					...
					try {
						if (pddest[j].getPropertyType().getName().equals("java.util.Date"))
							PropertyUtils.setProperty(dest, pddest[j].getName(),StringtoDate(value));
						if (pddest[j].getPropertyType().getName().equals("java.lang.Double"))
							PropertyUtils.setProperty(dest, pddest[j].getName(),StrToDouble(value));
						if (pddest[j].getPropertyType().getName().equals("java.lang.Integer"))
							PropertyUtils.setProperty(dest, pddest[j].getName(),StrToInteger(value));
						if (pddest[j].getPropertyType().getName().equals("java.lang.Long"))
							PropertyUtils.setProperty(dest, pddest[j].getName(),StrToLong(value));
	...
	/**
	 * pojo对象名称与属性值输出
	 * @param obj     pojo对象或存放pojo对象的list
	 * @param wholeWord   整字输出，ture是，false否，选择false只输出前30个文字
	 */
	...
	private static void organizStr(Object o, boolean wholeWord,
		...
		for (int j = 0; j < pdout.length; j++) {
			...
			if (test) {
				...
				try {
					// 将属性值 存放于object变量里
					Object getobj = PropertyUtils.getProperty(o, pdout[j]
							.getName());
					// 将属性值 追加字符中变量里
...
//		d.setDeptName("name");
//		d.setDeptPid("pid");
//		d.setDeptDescription("description");
//		
//		Set<UserList> set=new HashSet<UserList>();
