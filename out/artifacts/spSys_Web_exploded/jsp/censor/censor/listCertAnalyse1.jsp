...
// Radialize the colors
    ...
    $('#highchartDivCheck15').highcharts({       
     //credits: '啊啊啊啊',
        //图表区选项
        ...
        plotOptions: {
            pie: {
                ...
                events:{//监听点的鼠标事件  
                    click: function(e) {
                    	$("#detail1").empty();
                    	$("#detail1").append("<tr><td>扣分规则</td><td>单位名称</td><td>扣分</td></tr>");
	                    var strss = "${divScore}";
		            	var strs = strss.split(";");
		            	for(var i=0;i<strs.length;i++){
		            		var str = strs[i].split(",");
		            		if(e.point.name==str[0]){
		            			if(str[1] == 'null'){
		            				$("#detail1").append("<tr><td>"+str[0]+"</td><td>"+str[2]+"</td><td> </td></tr>");
		            			}else{
		            				$("#detail1").append("<tr><td>"+str[0]+"</td><td>"+str[2]+"</td><td>扣"+str[1]+"分</td></tr>");
		            			}
        ...
        function showResult(){
	       var pkUnita = $("#pkUnita").val();
			...
			$("#highchartDivCheck15").loadUrl("listCertAnalyse1.do?pkUnita="+pkUnita+"&pkUnitb="+pkUnitb+"&pkUnitc="+pkUnitc+"&pkUnitd="+pkUnitd+"&year="+year+"&certType="+certType+"&yearTo="+yearTo);  
        }
