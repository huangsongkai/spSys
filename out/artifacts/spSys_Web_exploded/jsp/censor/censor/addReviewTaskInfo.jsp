	...
	<div align="center"  layOutH="138">
		...
		<table class="list" width="100%" id="certTable">
			...
			<tr>
				...
				<td align="left">
					<input name="certTaskInfo.pkUnit" type="hidden" value="${CertTaskInfo.pkUnit}">
			...
			<tr>
				...
				<td align="left">
					&nbsp;&nbsp;<select onchange="changeTextArea2()" style="width: 154" name="certTaskInfo.confidentialityLevelQualifications" id="confidentialityLevelQualifications" value="${certTaskInfo.confidentialityLevelQualifications}" dataType="Require" msg="资格等级不能为空">
			...
			<tr>
				...
				<td align="left">
						&nbsp;&nbsp;<select name="certTaskInfo.reviewResult"  style="width: 154" id="reviewResult" value="${certTaskInfo.reviewResult}" dataType="Require" msg="复查结论不能为空">
						<c:forEach items="${applicationScope.CodeDict}" var="item">
						 	<c:if test="${item.parentCodeId=='020'}">  
							 		<option value="${item.codeId}" <c:if test="${item.codeId==certTaskInfo.reviewResult}">selected</c:if> >${item.codeName}</option>
						 	</c:if>
						</c:forEach>
					</select>
				</td>
			...
			<tr>
				...
				复<br>审<br>组<br>组<br>成<br>人<br>员				
				</td>				
				<td width="20%">
					姓名
				</td>
					...
					<td align="left" colspan="5">
						...
						<table id="tb${count.count}">
							...
							<tr style="display:none">
								...
								<td>
									<input name="scoreRule" class="noborderInput" size="55">
								</td>
								<td>
									扣<input name="scoreNumber" id="${count.count}${status.count}" onchange="scoreNumberOnChange(this.id)" class="noborderInput" size="2">分
								</td> 
			...
			<tr>
				<td align="left" colspan="5">
					审查组意见：<br>
					&nbsp;&nbsp;&nbsp;&nbsp;
					经研究，公司申请理由充分，现场审查实有项目总分值
					<input type="text" name="total" size="2" readonly="readonly" class="noborderInput" style="border-bottom-style:none"/>
					分，实际得分
					<input type="text" name="certTaskInfo.passedScore" id="passedScoreReview" size="2" readonly="readonly" value="${certTaskInfo.passedScore}" class="noborderInput" style="border-bottom-style:none"/>
					分。
					<textarea name="certTaskInfo.textarea2" rows="2" cols="100"><c:if test="${certTaskInfo.textarea2!=null}">${certTaskInfo.textarea2}</c:if></textarea>
					<br>&nbsp;&nbsp;&nbsp;&nbsp;
					审查组组长（签名）：
					<br><br>&nbsp;&nbsp;&nbsp;&nbsp;					
				</td>
