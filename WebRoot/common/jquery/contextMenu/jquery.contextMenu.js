...
var // currently active contextMenu trigger
    ...
    handle = {
        ...
        mousedown: function(e) {
        	//document.getElementById("trid").innerHTML=$(this).html();

            // register mouse down
            var $this = $(this);
            xjContextMenu.mouseDownEvent($(this));
           	/*
...
var xjContextMenu = {
	el:null,
	trcnt : 0 ,
	mouseDownEvent : function(el,menuId)
	{
		xjContextMenu.el = el;
	

	},
	beforeMouseUpEvent : function()
	{
			//$("#messages").text("mouse up before");
//		alert("-------------------mouse up before");
	},
	//初始化表格
	initContextTable : function(tableId)
	{
				// Show menu when a list item is clicked
				 $.contextMenu(
				 {
					 ...
					 callback: function(key, options) {
					 					var el = xjContextMenu.el;
		                if(key=="insert_up"){
											$(el).before("<tr id=\"autoTrId"+xjContextMenu.trcnt+"\">"+$("#insertContent").val()+"</tr>");
											xjContextMenu.insertUpCallBack("autoTrId"+xjContextMenu.trcnt);
										}else if(key=="insert_down"){
											$(el).after("<tr id=\"autoTrId"+xjContextMenu.trcnt+"\">"+$("#insertContent").val()+"</tr>");
											xjContextMenu.insertDownCallBack("autoTrId"+xjContextMenu.trcnt);
										}else if(key=="copy_up"){
											$(el).before("<tr id=\"autoTrId"+xjContextMenu.trcnt+"\">"+$(el).html()+"</tr>");
											xjContextMenu.copyUpCallBack("autoTrId"+xjContextMenu.trcnt);
										}else if(key=="copy_down"){
											$(el).after("<tr id=\"autoTrId"+xjContextMenu.trcnt+"\">"+$(el).html()+"</tr>");
											xjContextMenu.copyDownCallBack("autoTrId"+xjContextMenu.trcnt);
										}else if(key=="delete"){
											var elId = $(el).attr("id");
											$(el).remove();
											xjContextMenu.deleteCallBack(elId);
										}
		              
		                    var m = "edit was clicked:"+key ;
		               //     window.console && console.log(m) || alert(m); 
		                    xjContextMenu.trcnt=xjContextMenu.trcnt+1;
		        },
						items: {
		            "insert_up": {
		                name: "在上方插入行", 
      	...
      	$.contextMenu(
				 {
					 selector: ".tbl .td_title",
					 callback: function(key, options) {
					 					var el = xjContextMenu.el;
		                if(key=="insert_down"){
											$(el).after("<tr id=\"autoTrId"+xjContextMenu.trcnt+"\">"+$("#insertContent").val()+"</tr>");
											xjContextMenu.initContextTable(tableId);
											xjContextMenu.insertDownCallBack("autoTrId"+xjContextMenu.trcnt);
										}
	...
	insertUpCallBack : function(trId)
	{
		//向上插入行后的回调函数，传入所增加行的ID

	},insertDownCallBack : function(trId)
	{
		//向下插入行后的回调函数
	},copyUpCallBack : function(trId)
	{
		//向上复制行后的回调函数
	},copyDownCallBack : function(trId)
	{
		//向下复制行后的回调函数
	},deleteCallBack : function(trId)
	{
		//删除行后的回调函数
	}
}